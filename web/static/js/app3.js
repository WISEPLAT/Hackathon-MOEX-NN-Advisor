...
//  if (val >= 0 && val < 3) {color = colors[3];}
  ...
  if (val <= 0.45) {color = colors[1];}
  if (val <= 0.4) {color = colors[0];}
  if (val > 0.45 && val < 0.5) {color = colors[2];}

  if (val >= 0.55) {color = colors[4];}
  if (val >= 0.6) {color = colors[5];}
  if (val >= 0.5 && val < 0.55) {color = colors[3];}
...
//      .attr("fill", (d) => getColor(d.data.pc))
      .attr("fill", (d) => getColor(d.data.predict))
